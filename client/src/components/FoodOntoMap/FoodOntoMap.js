...
class FoodOntoMap extends React.Component {
    ...
    displayTags = (key, dataset) => {
        ...
        if (this.state.searchResults[key]) {
            ...
            let results = keyResults[dataset] || [];
            ...
            let tagDisplay = tags.map(t => ['', t]).map(this.displayTag(dsMap, 'text-dark', dataset));
            return tagDisplay;
        }
    ...
    displayTag = (dsMap, clazz, dataset) => {
        return (tag) => {
            return (<a key={tag[1]} target="__blank" href={dsNamespaces[dataset].url + tag[0]}>
