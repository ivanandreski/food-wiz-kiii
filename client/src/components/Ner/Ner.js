...
class Ner extends React.Component {
  ...
  nerCharDisplay = (token) => {
    const badge = token.removed ? 'entity badge badge-danger' : 'entity badge badge-success'
    return (
  ...
  visualizeEntityAnnotations = (e, ix) => {
    ...
    if (!curated) {
      removeCurated = (
        <span
          title='Remove Tag' onClick={() => this.removeEntity(e)}
          className='btn btn-danger btn-xs clickable'
        >x
        </span>
      )
    ...
    return (
      <tr className={this.highlightClass(e.start)} key={ix}>
        <td>
          {e.word}
          {removeCurated}
        </td>
